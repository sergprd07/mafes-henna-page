...
if (bloomWatermarks && !reduceMotion) {
  ...
  const updateBloomFromDelta = (delta) => {
    bloom = Math.min(Math.max(bloom + delta / 720, 0), 1);
    if (!ticking) {
  ...
  const updateBloomFromScroll = () => {
    const currentScrollY = window.scrollY;
    const delta = currentScrollY - lastScrollY;
    lastScrollY = currentScrollY;
    updateBloomFromDelta(delta);
  };

  bloomWatermarks.style.setProperty("--bloom", bloom.toFixed(4));
  window.addEventListener("scroll", updateBloomFromScroll, { passive: true });
}
