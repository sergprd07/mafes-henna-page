...
const scrubBloomVideos = () => {
  bloomVideos.forEach((video) => {
    ...
    const targetTime = Math.min(video.duration - 0.04, Math.max(0, bloomScrubProgress * video.duration));
...
bloomVideos.forEach((video) => {
  video.pause();
  video.addEventListener("loadedmetadata", scrubBloomVideos, { once: true });
});
...
if (bloomWatermarks && !reduceMotion) {
  let ticking = false;
  const cycleLength = 980;

  const applyBloom = () => {
    const cycle = ((window.scrollY % cycleLength) + cycleLength) % cycleLength;
    const phase = cycle / cycleLength;
    const bloom = phase < 0.5 ? phase * 2 : (1 - phase) * 2;
    const unbloom = 1 - bloom;

    bloomWatermarks.style.setProperty("--bloom", bloom.toFixed(4));
    bloomWatermarks.style.setProperty("--unbloom", unbloom.toFixed(4));
    bloomWatermarks.style.setProperty("--bloom-drift", `${(window.scrollY * 0.18).toFixed(2)}px`);
    bloomScrubProgress = bloom;
    scrubBloomVideos();
    ticking = false;
  };
